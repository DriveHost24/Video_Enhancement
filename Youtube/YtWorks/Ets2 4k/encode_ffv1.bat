...
REM ---------------- CONFIG ----------------
set "SRC=input.mov"
set "OUT_FOLDER=ETS2_4K_VERTICAL_FFV1_LOSSLESS"
if not exist "%OUT_FOLDER%" mkdir "%OUT_FOLDER%"

set "OUT_W=2160"
set "OUT_H=3840"
set "FPS=60"
set "PIX_FMT=yuv422p10le"

REM ---------------- METADATA ----------------
set "META_TITLE=Euro Truck Simulator 2 Volvo FH6 Aero Globetrotter XL 780 hp 574kW 12 speeds 2025-11-10 11-46-12"
...
for %%F in (tmp_segments\clip_*.mov) do (
    ...
    ffmpeg -y -i "%%F" ^
      ...
      -slices 24 -slicecrc 1 -threads 8 ^
      -pix_fmt %PIX_FMT% ^
      -c:a pcm_s16le ^
      ...
      "%OUT_FOLDER%\!BASE!_4K60_vertical_ffv1_lossless.mkv"

    timeout /t 1 >nul
)

echo ✅ Done. All 4K60 vertical FFV1 lossless clips saved in: %OUT_FOLDER%
pause
